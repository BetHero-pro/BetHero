...
function addToLocalStorePlaylist(newQuest, currentPlaylistName) {
  const playlists = JSON.parse(localStorage.getItem('playlists')) || [];
  const index = playlists.findIndex(playlist => playlist.name === currentPlaylistName);
  if (index !== -1) {
    playlists[index].quests.push(newQuest);
...
const PlaylistMainPage = () => {
  ...
  // TODO: replace 'currentPlaylist' with backend playlist
  const location = useLocation();
  const [currentPlaylist, setCurrentPlaylist] = useState(location.state.playlist);
  const [questions, setQuestions] = useState(location.state.playlist.quests);

  // Function to update the questions state based on localStorage
  const updateQuestions = () => {
    const playlists = JSON.parse(localStorage.getItem('playlists')) || [];
    const updatedPlaylist = playlists.find(playlist => playlist.name === currentPlaylist.name);
    if (updatedPlaylist) {
      setCurrentPlaylist(updatedPlaylist);
      setQuestions(updatedPlaylist.quests);
    }
  }

  // Watch for changes in 'playlists' in localStorage
  useEffect(() => {
    window.addEventListener('storage', updateQuestions);

    // Cleanup on unmount
    return () => window.removeEventListener('storage', updateQuestions);
  }, []);
  ...
  const handleAddQuest = async state => {
    // TODO: Replace localStorage with backend
    if (state.status === true) {
      const newQuest = {
        _id: generateUUID(),
        Quest: state.newQuestion,
      ...
      addToLocalStorePlaylist(newQuest, currentPlaylist.name);
      setDisabledButton(false);
      toggleRefresh(!refresh);
      setIsQuestion(false);
    } else {
        ...
        <>
          <div className="bg-blue-100 w-screen h-screen">
            ...
            <div className="flex justify-center space-x-2">
              ...
              <button className='w-13 h-13 cursor-pointer border border-black rounded-full bg-green-500 hover:bg-gray-500' onClick={startPlaylistTask}><img src="/playlist.png" className='w-10 h-10 p-2' /></button>
